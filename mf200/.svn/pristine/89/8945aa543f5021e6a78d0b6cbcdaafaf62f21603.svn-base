...
/*			if($group_id){
		...
		if($class == 2){
			...
			$info = Db::name('bill')->where($con)->where($where)->field('b_id,add_time,type,worker_id,check_worker_1')->order('add_time desc,b_id desc')->select();

		}else{

			$info = Db::name('bill')->where($con)->field('b_id,add_time,type,worker_id,check_worker_1')->order('add_time desc,b_id desc')->select();
		}
		if($worker_name){
