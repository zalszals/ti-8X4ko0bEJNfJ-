...
/*		if(!$check_worker_2){
	...
	public  function over_list(){
		...
		$info = Db::name('bill')->where($con)->field('b_id,worker_id,check_worker_1,check_worker_2,check_time_1,check_time_2,add_time,reason,day,status,type,s_time,e_time')->order('add_time desc')->select();
