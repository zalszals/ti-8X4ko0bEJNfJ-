...
class WorkerRecord extends Base{
	...
	public function time_list(){
		...
		if($find){
			...
			return json(['status'=>1,'msg'=>'查询成功','data'=>$find]);
	...
	public function record_list(){
		...
		if($class == 2){
			...
			$where = " FIND_IN_SET($w_id ,worker_code)";
		}else{
			$where = '';
		}
		...
		if(isset($worker_name) && $worker_name){
			...
			$con['worker_name'] = array('like','%'.trim($worker_name).'%');
		}

		$con['status'] = 1;

		$worker = Db::name('worker')->field('worker_id,worker_name,pid,role_id,group_id')->where($con)->where($where)->order('worker_id')->select();
		...
		foreach($worker as $k=>$v){
			...
			if($v['role_id']){
				...
				if(isset($role_name) && $role_name){
					...
					$con1['role_name'] = array('like','%'.trim($role_name).'%');
				}
				$con1['role_id'] = $v['role_id'];
				
				$worker[$k]['role_name'] = Db::name('role')->where($con1)->value('role_name');
			...
			$worker[$k]['group_name'] = Db::name('group')->where($con2)->value('group_name');

			if(isset($worker[$k]['role_name'])  && isset($worker[$k]['group_name'])){
	...
	public function record_list_detail(){
		...
		return json(['status'=>1,'msg'=>'查询成功','data'=>$arr3]);
