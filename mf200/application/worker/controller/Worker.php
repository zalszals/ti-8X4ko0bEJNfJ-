...
class Worker extends Base
{
    ...
    public function workerlist()
    {
        ...
        if(isset($worker_name)){
            $con['mf_worker.worker_name'] = array('like','%'.trim($worker_name).'%');
        }
        ...
        $con['mf_worker.status'] = 1;
        $con['mf_worker.group_id'] = $group_id;
            
        $sql_list = Db::view('mf_worker', 'worker_id,pid,phone,group_id,worker_name,sfz,sex,price,bank,bank_account,nation,address,birth_date,age,entry_date,worker_no,role_id')
            ->view('mf_group', 'group_name', 'mf_group.group_id=mf_worker.group_id')
            ->view('mf_role','role_name','mf_role.role_id=mf_worker.role_id')
            ->where($con)
    ...
    public function workerselect()
    {
        ...
        if ($group_id || $selectinput) {
            ...
            if (! empty($selectinput)) {
                ...
                $data['mf_worker.group_id'] = $group_id;
                // $data['role_id']=$role_id;
                
                $data['mf_worker.status'] = 1;
    ...
    public function workeredit_a()
    {
        ...
        if (is_numeric($edit_id)) {
            ...
            $people_sel = Db::view('mf_worker', 'worker_id,pid,group_id,worker_name')->view('mf_role', 'role_name', 'mf_role.role_id=mf_worker.role_id')
                ->where('group_id', $group_id)
                ->select();
            ...
            $editsql = Db::view('mf_worker', 'phone,group_id,sex,worker_name,sfz,price,bank,bank_account,address,nation')
