...
public function product_list_detail(){
	...
	/* foreach($data as $k=>$info){ */
		foreach($info as $k1=>$v1){
			...
			$info['add_time'] = date('Y-m-d',strtotime($info['add_time']));
	...
	public function product_list(){
		...
		if($s_time){
			$condition['pp.add_time'] = array('egt',date('Y-m-d 00:00:00',strtotime($s_time)));
		}
		if($e_time){
			$condition['pp.add_time'] = array('elt',date('Y-m-d 24:00:00',strtotime($e_time)));
		}
		if($s_time && $e_time){
			$condition['pp.add_time'] = array(array('egt',date('Y-m-d 00:00:00',strtotime($s_time))),array('elt',date('Y-m-d 24:00:00',strtotime($e_time))));
		}
		if($worker_name){
			$condition['w.worker_name'] = array('like',"%{$worker_name}%");
		}
